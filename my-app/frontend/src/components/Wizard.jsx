...
export default function Wizard() {
  ...
  return (
    <>
      ...
      <Container maxWidth="md" sx={{ py: 3, px: isSmallScreen ? 2 : 0, position: "relative" }}>
        {/* HEADER DEL WIZARD (oculto en plantillas) */}
        {!isTemplateStep && (
          <Box sx={{ display: "flex", alignItems: "center", mb: 3, width: "100%" }}>
            <IconButton onClick={handleLanding} color="primary" sx={{ mr: 2, p: 1 }}>
              <ArrowBackIcon />
            </IconButton>
        ...
        {/* CONTENIDO */}
        <Box sx={{ minHeight: 250, mb: 3 }}>{renderStep()}</Box>

        {/* BOTONES (ocultos en plantillas) */}
        {!isTemplateStep && (
          <Box
            sx={{
              display: "flex",
